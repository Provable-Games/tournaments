...
fn test_submit_multiple_scores_stress_test() {
    ...
    loop {
        ...
        // Give each player a score (higher index = higher score)
        contracts.minigame.end_game(*token_ids.at(i.try_into().unwrap()), (150 - i).try_into().unwrap());
        i += 1;
    };
    ...
    loop {
        ...
        // Submit in reverse order (highest scores first)
        let position: u8 = (i + 1).try_into().unwrap();
        contracts.budokan.submit_score(tournament.id, *token_ids.at((149 - i).try_into().unwrap()), position);
        i += 1;
    };
...
fn test_distribute_many_prizes() {
    ...
    loop {
        ...
        contracts.minigame.end_game(*token_ids.at(i.try_into().unwrap()), (250 - i).try_into().unwrap());
        i += 1;
    };

    // Submit scores to create leaderboard
    let mut i: u64 = 0;
    loop {
        if i == 250 {
            break;
        ...
        let position: u8 = (i + 1).try_into().unwrap();
        contracts.budokan.submit_score(tournament.id, *token_ids.at((249 - i).try_into().unwrap()), position);
        i += 1;
    };
